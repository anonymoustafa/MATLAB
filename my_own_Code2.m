...
Length_River    = xlsread(filename, 'River Propertise','B2:B2');
...
velocity_segment= zeros(segment_num,1);
...
%%  LDC computation
   ...
   for i=1:1:segment_num
      ...
      velocity_segment(i,1) = flow_Q(i,1)/Area_segment(i,1);
   end
clear i;
...
%% main solver
delta_T =   (Length_River / mean(velocity_segment))/segment_num;
for t = 1 : 1: segment_num-1
         
     for x = 2 : 1 : segment_num-1
             
          c0(t+1,x) = ...
       ((disscharge_mass(x,1) * delta_T) / vol_segment(x,1)) - ...
       (delta_T/vol_segment(x,1))*(-1*flow_Q(x,1)*Alphan-LCD0(x-1,1))*c0(t,x-1) ...
      +(1-(delta_T/vol_segment(x,1))*(-1*flow_Q(x,1)*Betha+flow_Q(x,1)*Alphan+LCD0(x-1,1)+LCD0(x,1))*c0(t,x)...
      -(delta_T/vol_segment(x,1))*(flow_Q(x,1)*Betha-LCD0(x,1))*c0(t,x+1));
