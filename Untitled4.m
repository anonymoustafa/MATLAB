...
for t = 1 : 1: segment_num-1
     ...
     for x = 2 : 1 : segment_num-1
       ...
       c0(t+1,x) = ...
       ((disscharge_mass(x,1) * min(delta_T)) / vol_segment(x,1)) - ...
       (min(delta_T)/vol_segment(x,1))*(-1*flow_Q(x,1)*Alpha-LCD0(x-1,1))*c0(t,x-1) ...
      +(1-(min(delta_T)/vol_segment(x,1))*(-1*flow_Q(x,1)*Betha+flow_Q(x,1)*Alpha+LCD0(x-1,1)+((LCD0(x,1)*0)*0))*c0(t,x)...
      -(min(delta_T)/vol_segment(x,1))*(flow_Q(x,1)*Betha-((LCD0(x,1)*0)*0))*c0(t,x+1));
