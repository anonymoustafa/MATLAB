...
for t = 1 : 1: segment_num-1
     ...
     for x = 2 : 1 : segment_num-1
       ...
       c0(t+1,x) = ...
       ((disscharge_mass(x,1) * delta_T(1,1)) / vol_segment(x,1)) - ...
       (delta_T(1,1)/vol_segment(x,1))*(-1*flow_Q(x,1)*Alpha-0)*c0(t,x-1) ...
      +(1-(delta_T(1,1)/vol_segment(x,1))*(-1*flow_Q(x,1)*Betha+flow_Q(x,1)*Alpha+0+0)*c0(t,x)...
      -(delta_T(x,1)/vol_segment(x,1))*(flow_Q(x,1)*Betha-0*(x,1))*c0(t,x+1));
